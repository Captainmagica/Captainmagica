<template>
    <el-table :data="test.testArrangements" :cell-style="{ textAlign: 'center'}" 
...
import { type FormRules, type FormInstance, ElMessage, dateEquals } from 'element-plus';
...
const currentInfo = reactive<TestInfo>({
    content: '',
    date: '',
})
const originInfo = reactive<TestInfo>({
    content: '',
    date: '',
})
...
const handleSelectionChange = (selection: []) => {
    test.selectedRows = selection;
};

const handleReset = () => {
  if (formRef.value) {
    formRef.value.resetFields('date'); // 重置表单数据和验证状态
    test.ResetForm(currentInfo, originInfo); // 重置表单数据
  }
};
...
const handleSubmit = async () => {
    if (formRef.value) {
        formRef.value.validate(async (valid) => {
            if (valid) {
                addDialogVisible.value = await test.OnSubmit(currentInfo, originInfo, id.value);
            } else {
...
const updateRules = reactive<FormRules>({
    date: [
        { required: true, validator: test.ValidateDate(currentInfo), trigger: 'change' },
    ],
});

const HandleDelete = (id: number) => {
    axios.delete(`http://localhost:5172/api/TestArrangements/${id}`).then(()=>{
        ElMessage.success('删除成功')
        test.UpdateTestArrangement()
    })
};
